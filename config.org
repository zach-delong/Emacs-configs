...
**** Customized variables
     :PROPERTIES:
     :header-args: :noweb-ref ido-variables-config
     :END:
     ...
     Flexible matching allows Ido to match strings when characters match but do not completely match together. Meaning that if you typed "math" but nothing contained the string "math" but "Management Theory" was in the set, the "Ma" in "management" and the "Th" in "theory" would match.
     
     #+BEGIN_SRC emacs-lisp :tangle no
       (ido-enable-flex-matching t)
     #+END_SRC

     If you have a file path at your point (cursor) and you want to open that file, IDO can try to pre-populate the prompt with the file path at point.
...
Projectile is a library to replace project.el (the built in project management tool). They have a ton of overlap between their functionality, but for now, I am preferring using Projectile because I like the fact that it provides tools for both compiling and testing code and allows easy customization of what commands run in each project.

Notice that on init, we are enabling the mode. This is so that it loads the list of "known projects" immediately on starting emacs, so I can easily immediately ~<leader> p p~ to switch to a project.

I also set a keybinding for saving projects to that list.
	...
	(evil-global-set-key 'normal (kbd "<leader> f p r") 'projectile-add-known-project))
    #+END_SRC
