...
First up, I have to make sure that [[https://repo.or.cz/w/emacs.git/blob_plain/HEAD:/lisp/emacs-lisp/package.el][package.el]] is configured. Package.el is a package manager that's built into GNU Emacs (apparently since Emacs24). I will be using ~use-package~ for the most part, which is a macro for installing packages if they aren't already installed. That said, if I want to install things, chances are pretty good that I want to install them from [[https://melpa.org/][MELPA]]. I would like to use [[http://elpa.gnu.org/][ELPA]], which is vetted by the FSF, but not everyone uploads there, so for now I use MELPA for most things.

package.el uses a priority system for picking what package archive to use locally.  I am setting that priority to MELPA > Non-GNU ELPA > ELPA, for the best likelihood of not having conflicting versions of things installed. 
...
** Use-package
   ...
   [[https://github.com/jwiegley/use-package/tree/a7422fb8ab1baee19adb2717b5b47b9c3812a84c#use-package][Use-package]] is a crazy macro that does a lot of things but I am primarily using it to lazy-load things (using ~package.el~) and to consolidate the list of plugins I am using with their configuration. It used to be that I had to do some bootstrapping to get ~use-package~ installed property before I could refresh packages and try to start installing things, but thankfully, ~use-package~ now comes with Emacs as of Emacs 29, so we can just assume it's installed and use it! 
...
* Configuration
  ...
  This is where the meat-and-potatoes of my configuration will live.
...
*** prettier-js mode
    ...
    Unfortunately, most projects these days use a "local" copy of prettier installed in the node_modules folder. There is a plugin that purports to do that, but it has a bug in current versions of node. I have tried to fix that myself locally. I should try to submit that upstream.
	...
	:config
	(message "Attempted to load add-local-node-modules"))
    #+END_SRC
