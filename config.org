...
#+BEGIN_SRC emacs-lisp
  (ert-deftest get-path-separator-should-work ()
    "The function should always return / in gnu/linux. I thought it
  shold return \ on Windows, but it also returns / there, so I am
  unsure what the expected behavior should be."
    (should (equal (get-path-separator) "/")))
#+END_SRC
...
**** Unit tests
    #+BEGIN_SRC emacs-lisp
      (ert-deftest get-file-name-from-relative-path ()
	"given a relative path with a file at the end of it, return the
      file's name."
	(should
	 (equal (get-file-name-from-path "relative/path/to/file.md")
		"file.md")))
    #+END_SRC

    #+BEGIN_SRC emacs-lisp
      (ert-deftest get-file-name-from-absolute-path ()
	"given an absolute path with a file at the end of it, return the
      file's name."
	(should
	 (equal (get-file-name-from-path "/this/is/a/relative/path/to/file.md")
		"file.md")))
    #+END_SRC

    #+BEGIN_SRC emacs-lisp
      (ert-deftest get-file-name-from-absolute-path-to-folder ()
	"given an absolute path with a to a folder, return the folder's
      name."
	(should
	 (equal (get-file-name-from-path "/this/is/a/relative/path/to/folder")
		"folder")))
    #+END_SRC
