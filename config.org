...
****** The junk broken out
:PROPERTIES:
:header-args: :noweb-ref junk-org-config
:END:
This part of the doc is an attempt to decompose the above block into smaller blocks that can be more easily documented. Eventually this will replace the above header with just a small block at the top. For now, though, have a fun duplicate header. Notice the properties on this header. It means that all the source blocks below this header will have that name, so we can reference them all at once.
