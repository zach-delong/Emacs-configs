...
***** Leader
      ...
      #+BEGIN_SRC emacs-lisp :tangle no
	(evil-set-leader '(normal visual replace operator emacs) (kbd "SPC"))
	(evil-set-leader '(normal visual replace operator emacs) (kbd "<leader> m") t)
      #+END_SRC
...
**** Require package
	 ...
	 ; Trying this out here: It sort of works (leader starts working) but
	 ; now I lose the hotkeys that are motion-state specific https://emacs.stackexchange.com/questions/71629/evil-normal-state-seems-to-have-no-effect-as-part-of-org-agenda-mode-hook
