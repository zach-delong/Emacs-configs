...
* What is this?
  ...
  This file is my attempt at literate programming Literate programming is a technique of writing documentation that explains why software should work a certain way, interspersed with examples that are the actual executable code. [fn:1] I have actually written about the practice before. [fn:2]

  Broadly, I have broken this file into several sections:
  1. [[*What is this?][What is this]] (you are here)
  2. [[*Pre-configuration][Pre-configuration]] (bootstrapping and shared config)
  3. [[*Sources][Sources]] (good books to read and inspiration)
  4. [[*Notes][Notes]] (misc things I wanted to note for future work)
  5. [[*Footnotes][Footnotes]] (who doesn't love footnotes?)
    
* Pre-configuration

  Before I actually configure anything interesting, I need to do  certain bootstrapping tasks (like install use-package). In this section, I do not  assume that anything has been installed, but configure things that  are generic (at least in theory) to any configuration of Emacs.
...
*** Initialization
    ...
    Before getting to setting up ~use-package~, I need to make sure we have a reasonably up to date package archive to use. To make sure that we can actually connect to things and install them, I'll start by checking to see if we have packages in our archive. If we do, then we can skip refreshing contents (until something else later on does that for us). [fn:3]
    ...
    ~package.el~ exposes methods for doing a lot of things. Here's a quick reference:
    - ~package-refresh-contents~ :: query the package archives set up in ~package-archives~ to get an updated list of packages available for installation
    - ~package-installed-p~ :: check to see if the name provided is installed currently, accepts a minimum version
    - ~package-install~ :: install the provided package

    There are a few things that I don't love about ~package.el~.
    - It doesn't expose any way to install a specific version only
    - There isn't any kind of an npm lockfile equivalent
    - It's very, very manual

    Luckily, some (not all) of those problems are solved by ~use-package~! The specific problem of installing specific versions and controlling updates is not something that anything (as far as I can tell) actually solves with ~package.el~. Supposedly ~straight.el~ does solve that, but I have yet to try it.
    
** Use-package

*** A brief introduction to use-package

    ~use-package~ has a ton of options, but it's fundamentally a macro for installing packages and adding them to your ~load-path~ automatically. Use package has a ton of options that I frequently forget how to use, so I want to explain a few of them here for future reference. One of the big benefits is that nothing is loaded by `use-package` until a provided feature is actually requested. This means that you can defer configuration easily!
...
** exec-path
   ...
   It bothers me, but if I launch Emacs from the GUI (as I almost always do) it's not getting the correct ~exec-path~ for me. So I have to write some stuff to make sure that it's going to get my LSPs and whatnot.
...
*** Company-mode
    ...
    [[http://company-mode.github.io/][Company mode]] is an auto complete plugin (*comp*-lete *any*-thing). [[https://elpa.gnu.org/devel/doc/company.html][You can learn more about Company mode here]]. I am still exploring how it can be used. Note the hooks set up below. I am only enabling company mode in modes that inherit from ~prog-mode~ because I don't want it popping up constantly while I am typing in org mode or in markdown mode.
...
**** company variables
     ...
     #+BEGIN_SRC emacs-lisp :tangle no
       (company-backends '(company-ya snippet
       			       company-capf
     ...
     #+BEGIN_SRC emacs-lisp :noweb yes :tangle config.el
       (use-package org
         ...
         :bind(("C-c a" . org-agenda)
       	("C-c c" . org-capture)
       	(:map org-mode-map
       	      ("C-c m o i s" . org-insert-src-block)))
         :custom
         (org-agenda-span 14)
...
**** org-ql 
     ...
     #+BEGIN_SRC emacs-lisp :tangle config.el
       (use-package org-ql
         :ensure t
         :bind (("C-c m o q d" . org-ql-find-in-org-directory)
       	 ("C-c m o q a" . org-ql-find-in-agenda)
       	 ("C-c m o q f" . org-ql-find)))
     #+END_SRC
...
*** Zettelkasten
    ...
    This is a plugin that isn't in MELPA for now, and I can't seem to convince it to load by adding it to ~load-path~ so I am instead going to manually require it in the preface so that happens before doing anything in ~use-package~.
    ...
    #+BEGIN_SRC emacs-lisp :tangle config.el
      (use-package zettelkasten-mode
        ...
        :load-path "plugins/zettelkasten"
...
*** project.el
    ...
    #+END_SRC
     ...
     #+BEGIN_SRC emacs-lisp :tangle config.el
       (use-package project
         :defer t
         :bind (("C-x p R" . project-remember-current)))
     #+END_SRC
*** Flyspell mode
...
[fn:3] [[https://chrismaiorana.com/emacs-guides/installing-packages/][Chris maiorana on installing packages/setting up use-package]]

[fn:2] [[https://zachdelong.com/topics/software-engineering-principles/literate-programming-org-babel-noweb.html][my blog on org-mode and literate programming]] 

[fn:1] [[https://en.wikipedia.org/wiki/Literate_programming][Literate programming in Wikipedia]] 
